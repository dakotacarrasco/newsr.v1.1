...
class DigestGenerator:
    ...
    def create_digest_prompt(self, articles, city_name, region):
        """Create a prompt for the LLM to generate a well-structured digest"""
        
        # Get the current date
        today = datetime.now().strftime('%A, %B %d, %Y')
        
        # Prepare article information for the prompt - remove identifying information
        article_info = []
        ...
        # Create the main prompt with improved instructions
        prompt = f"""
    ...
    def generate_digest_content(self, articles, city_name, region, topics=None):
        ...
        prompt = self.create_digest_prompt(articles, city_name, region)
    ...
    def generate_digest_for_city(self, articles, city_code, city_config=None):
        """Generate a digest for a specific city focusing on content"""
        if not city_config:
        ...
        # Use today's date for the digest
        today = datetime.now().strftime('%Y-%m-%d')
        
        # Prepare article dicts 
        article_dicts = self.prepare_articles(articles)
        
        # Generate the digest content with simplified approach
        digest_content, sections = self.generate_digest_content(
            article_dicts, 
            city_name, 
            region,
        ...
        # Get unique sources from the articles
        sources = []
        ...
        # Extract headline from content - use ONLY the headline from the LLM-generated content
        headline = ""
        first_line = digest_content.strip().split('\n')[0]
        if first_line.startswith('**[') and first_line.endswith(']**'):
            headline = first_line.strip('**[]')
        else:
            headline = f"{city_name} Daily News Update"
        ...
        digest = {
            ...
            'region': region,
            'date': today,
            'headline': headline,  # Using only the LLM-generated headline
        ...
        logger.info(f"Digest created with {len(article_dicts)} articles from sources: {sources}")
    ...
    def generate_from_directory(self, directory, city_code=None, output_dir="digests", custom_prefix=""):
        ...
        try:
            # Generate the digest
            digest = self.generate_digest_for_city(unique_articles, city_code, city_config)
