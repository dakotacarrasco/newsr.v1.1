...
# Import modules
from wrangler.citydigest.config.cities import CITIES, get_city_config
from .digest.digest_generator import DigestGenerator
from .db.supabase_integration import SupabaseIntegration
from wrangler.citydigest.scheduler.scheduler import CityScheduler
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("/app/logs/citydigest.log"),
...
def scrape_city(city_code, headless=True, output_dir=None):
    ...
    try:
        ...
        from wrangler.citydigest.scrapers.base_scraper import BaseCityScraper
...
def generate_digest(city_code, input_dir=None, output_dir=None, upload=False):
    ...
    try:
        ...
        if input_dir:
            # In Docker environment, check the mounted volume paths
            docker_paths = [
                f"/app/data/{city_code}_news",
                f"/app/data/output/{city_code}_news"
            ]
            
            # If the provided path doesn't exist, try the Docker paths
            if not os.path.exists(input_dir):
                for docker_path in docker_paths:
                    if os.path.exists(docker_path):
                        input_dir = docker_path
                        logger.info(f"Using Docker path: {input_dir}")
                        break
        else:
            # Default to Docker path if none provided
            input_dir = f"/app/data/{city_code}_news"
            if not os.path.exists(input_dir):
                input_dir = f"/app/data/output/{city_code}_news"
        
        # Set default output directory if not provided
        if not output_dir:
            output_dir = f"/app/data/digests/{city_code}"
